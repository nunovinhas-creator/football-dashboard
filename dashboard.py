...
def fetch_all_predictions():
    """Busca predicoes e filtra APENAS as do dia de hoje pelo event_date (UTC)."""
    today = today_str()
    # Também aceitar amanhã UTC (pode ser hoje em Portugal)
    ...
    while True:
        try:
            ...
            filtered = [
                r for r in results
                if r.get("event", {}).get("event_date", "")[:10] in valid_dates
                and r.get("event", {}).get("status", "") != "finished"
            ]
            all_preds.extend(filtered)
            print(f"  [fetch] offset={offset} -> {len(results)} total, {len(filtered)} validos")
