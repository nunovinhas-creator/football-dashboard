...
def match_card_html(enriched):
    ...
    return f'''
    <div class="card" data-league="{league}" data-hour="{ko_hour}" data-conf="{conf_label}" data-hw="{int(hw*100)}" data-dr="{int(dr*100)}" data-aw="{int(aw*100)}" data-o25="{int(o25*100)}" data-btts="{int(bt*100)}" data-xgtotal="{round((float(xg_h) if xg_h not in ['–',None] else 0) + (float(xg_a) if xg_a not in ['–',None] else 0), 2)}">
      <div class="card-top">
...
function getMarketScore(c) {{
  ...
  if (activeMarket === "1x2")  return Math.max(+c.dataset.hw, +c.dataset.dr, +c.dataset.aw);
  if (activeMarket === "o25")  return +c.dataset.o25;
  if (activeMarket === "btts") return +c.dataset.btts;
  if (activeMarket === "xg")   return +c.dataset.xgtotal;
  return 0;
}}
...
function passesMarketFilter(c) {{
  ...
  if (activeMarket === "1x2")  return Math.max(+c.dataset.hw, +c.dataset.dr, +c.dataset.aw) >= 61;
  if (activeMarket === "o25")  return +c.dataset.o25 >= 61;
  if (activeMarket === "btts") return +c.dataset.btts >= 61;
  return true;
}}
...
function applyFilters() {{
  ...
  // Reordenar por score do mercado activo (maior primeiro)
...
function toggleConf(val) {{
  ...
  Object.values(CONF_MAP).forEach(b => document.getElementById("btn-"+b).classList.remove("active"));
  if (activeConf) document.getElementById("btn-"+CONF_MAP[activeConf]).classList.add("active");
  applyFilters();
}}
