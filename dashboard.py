...
def match_card_html(enriched):
    ...
    return f'''
    <div class="card" data-league="{league}" data-hour="{ko_hour}" data-conf="{conf_label}">
      <div class="card-top">
...
<div class="header">
  ...
  <div class="header-meta">Actualizado em {now} · Modelo BSD CatBoost v5.0</div>
</div>
...
function applyFilters() {{
  ...
  document.querySelectorAll(".card").forEach(c => {{
    const cl = c.dataset.league;
    const ch = c.dataset.hour;
    const cc = c.dataset.conf;
    const okL = !league || cl === league;
    const okH = !hour   || ch === hour;
    const okC = !activeConf || cc === activeConf;
    const show = okL && okH && okC;
