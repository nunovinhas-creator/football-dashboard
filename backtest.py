...
Modo SAVE  (07:00 UTC): guarda predições do dia em docs/preds_YYYY-MM-DD.json
Modo SCORE (00:00 UTC): cruza predições de ontem com resultados reais e acumula
"""
...
# ── Converter predição + resultado em registo ─────────────────
...
def build_html(history):
    ...
    return (
        ...
        f'<script>window.dataLayer=window.dataLayer||[];function gtag(){{dataLayer.push(arguments)}}gtag("js",new Date());gtag("config","G-WE48R4KL96");</script>'
        f'<style>'
        f':root{{--bg:#0d1117;--card:#1c2333;--border:#2d3748;--blue:#60a5fa;--green:#4ade80;--yellow:#fbbf24;--red:#f87171;--text:#f1f5f9;--sub:#94a3b8;--muted:#4a5568}}'
        f'*{{box-sizing:border-box;margin:0;padding:0}}body{{background:var(--bg);color:var(--text);font-family:"Inter","Segoe UI",system-ui,sans-serif}}'
        f'.hdr{{background:linear-gradient(180deg,#0a0f1e,#0d1117);border-bottom:1px solid var(--border);padding:20px 28px}}'
        f'.hdr h1{{font-size:1.5rem;font-weight:800;background:linear-gradient(90deg,#60a5fa,#a78bfa);-webkit-background-clip:text;-webkit-text-fill-color:transparent}}'
        f'.hdr .meta{{font-size:.72rem;color:var(--muted);margin-top:4px}}'
        f'.tabs{{display:flex;background:#0a0f1e;border-bottom:1px solid var(--border);padding:0 28px}}'
        f'.tab{{padding:12px 20px;font-size:.82rem;font-weight:600;color:var(--muted);border-bottom:2px solid transparent;text-decoration:none;transition:all .15s}}'
        f'.tab:hover{{color:var(--sub)}}.tab.active{{color:var(--blue);border-bottom-color:var(--blue)}}'
        f'.wrap{{max-width:960px;margin:0 auto;padding:24px 28px}}'
        f'.info{{background:#161b27;border:1px solid var(--border);border-radius:10px;padding:14px 20px;margin-bottom:24px;display:flex;gap:20px;flex-wrap:wrap;align-items:center;font-size:.8rem;color:var(--sub)}}'
        f'.info b{{color:var(--text)}}.grow{{margin-left:auto;font-size:.72rem;color:var(--muted)}}'
        f'.stitle{{font-size:.85rem;font-weight:700;color:var(--sub);margin:0 0 14px;text-transform:uppercase;letter-spacing:.5px;padding-left:10px;border-left:3px solid var(--blue)}}'
        f'.grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:14px;margin-bottom:28px}}'
        f'.lgrid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:14px}}'
        f'.sc,.lc{{background:var(--card);border:1px solid var(--border);border-radius:12px;padding:18px}}'
        f'.sc-top{{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px}}'
        f'.sc-title{{font-size:1rem;font-weight:700}}.sc-sub{{font-size:.72rem;color:var(--muted);margin-top:3px}}'
        f'.sc-rate{{font-size:1.8rem;font-weight:800;line-height:1}}'
        f'.rbg{{height:5px;background:var(--border);border-radius:3px;margin-bottom:14px}}'
        f'.rf{{height:100%;border-radius:3px}}'
        f'.tw{{margin-bottom:12px}}.tlbl{{font-size:.65rem;color:var(--muted);text-transform:uppercase;letter-spacing:.4px;margin-bottom:6px}}'
        f'.tbars{{display:flex;align-items:flex-end;gap:4px;height:52px}}'
        f'.tb{{flex:1;border-radius:3px 3px 0 0;min-width:8px;cursor:pointer}}'
        f'.ct{{width:100%;border-collapse:collapse;font-size:.78rem;margin-top:10px}}'
        f'.ct th{{text-align:left;color:var(--muted);padding:5px 6px;font-size:.65rem;text-transform:uppercase;letter-spacing:.4px;border-bottom:1px solid var(--border)}}'
        f'.ct td{{padding:6px 6px;border-bottom:1px solid #1a1f2e}}.ct tr:last-child td{{border-bottom:none}}'
        f'.tdc{{color:var(--sub)}}.tdl{{color:var(--text);max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}}.tdn{{text-align:right}}'
        f'.xgg{{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:14px 0}}'
        f'.xgi{{background:#0f1420;border:1px solid var(--border);border-radius:8px;padding:12px;text-align:center}}'
        f'.xgv{{font-size:1.3rem;font-weight:800;color:var(--green)}}.xgl{{font-size:.65rem;color:var(--muted);margin-top:4px}}'
        f'.lct{{font-size:.78rem;font-weight:700;color:var(--sub);margin-bottom:12px;text-transform:uppercase;letter-spacing:.4px}}'
        f'.nd{{font-size:.78rem;color:var(--muted);font-style:italic;padding:8px 0}}'
        f'.empty{{text-align:center;padding:60px 20px;color:var(--muted)}}.ebig{{font-size:3rem;margin-bottom:12px}}'
        f'.empty p{{font-size:.88rem;line-height:1.8}}'
        f'.footer{{text-align:center;padding:28px;font-size:.68rem;color:var(--muted);border-top:1px solid var(--border)}}'
        f'@media(max-width:580px){{.wrap,.hdr{{padding-left:14px;padding-right:14px}}}}'
        f'</style></head><body>'
        f'<div class="hdr"><h1>⚽ Matemática Da Bola</h1><div class="meta">Backtest actualizado em {now}</div></div>'
        f'<div class="tabs">'
        f'<a href="dashboard.html" class="tab">📊 Dashboard</a>'
        f'<a href="backtest.html" class="tab active">🔬 Backtest</a>'
        f'</div>'
        f'<div class="wrap">{body}</div>'
        f'<div class="footer">Matemática Da Bola · Backtest · Dados acumulados desde {date_min}</div>'
        f'</body></html>'
    )
...
def main():
    ...
    # MODO SAVE — guardar predições do dia (runs das 07:00, 14:00, 21:00)
    save_file = preds_file(today)
    if not os.path.exists(save_file):
        print(f"[backtest] SAVE: a guardar predições de {today}...")
        preds = fetch_todays_predictions()
        print(f"[backtest] {len(preds)} predições guardadas para {today}")
        if preds:
    ...
    # MODO SCORE — cruzar predições de ontem com resultados (run das 00:00 ou force)
    processed = history.get("dates_processed", [])
    if yesterday not in processed:
        yfile = preds_file(yesterday)
        if os.path.exists(yfile):
            print(f"[backtest] SCORE: a processar {yesterday}...")
            with open(yfile, encoding="utf-8") as f:
                preds = json.load(f)
            print(f"[backtest] {len(preds)} predições carregadas de {yesterday}")
            ...
            print(f"[backtest] SCORE: ficheiro {yfile} não existe — predições de ontem não foram guardadas")
            # Marca como processado para não tentar de novo
            history["dates_processed"] = processed + [yesterday]
            save_history(history)
    else:
        print(f"[backtest] SCORE: {yesterday} já processado")
