...
def fetch_finished_events(date_str):
    """Busca resultados reais do endpoint /events/ para uma data."""
    events = {}
    offset = 0
    while True:
        try:
            ...
            print(f"  [WARN] events offset={offset}: {ex}")
            break
    return events
...
def main():
    ...
    if yesterday not in processed:
        # 1. Buscar resultados reais do endpoint /events/
        print(f"[backtest] A buscar resultados reais de {yesterday}...")
        events = fetch_finished_events(yesterday)
        print(f"[backtest] {len(events)} eventos com resultado encontrados")

        # 2. Buscar predições dessa data
        ...
        # 3. Cruzar predições com resultados
        new_records = []
        for p in preds:
            eid = p.get("event", {}).get("id")
            if eid in events:
                new_records.append(make_record(p, events[eid]))

        print(f"[backtest] {len(new_records)} jogos com predição E resultado cruzados")
